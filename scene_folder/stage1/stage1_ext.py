...
ext = Character([
    [load_image(f'source\\character\\ext\\ext04_0{i}.png') for i in range(1, 3)],
    [load_image(f'source\\character\\ext\\ext04_0{i}.png') for i in range(3, 5)],
    [load_image(f'source\\character\\ext\\ext04_0{i}.png') for i in range(5, 8)]
], 100, 400, [],name = "EXT",flip=True)

ext.illust = load_image('source\\character\\ext\\hero_illust_14_Ext.png')

ext.status = {"nowhp": 100, "maxhp":790, "atk": 50, "def":20, "speed": 105, "condition":[], "origin_atk":110, "origin_def" : 70, "origin_speed":120}  # nowhp, maxhp, attack, speed
