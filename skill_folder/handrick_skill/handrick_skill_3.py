...
def create_skill_3(x, y, flip):
    skill = Skill(
        [load_image(f'source\\skill\\handrick\\40013_s2_0{i}.png') for i in range(1, 6)],
    ...
    def Draw_override(self):
        if self.visible:
            if self.flip == False:
                self.anime[self.frame % len(self.anime)].clip_draw(0, 0, self.anime[self.frame% len(self.anime)].w, self.anime[self.frame% len(self.anime)].h, self.x, self.y, 300, 110)
            elif self.flip == True:
                self.anime[self.frame % len(self.anime)].clip_composite_draw(0, 0, self.anime[self.frame % len(self.anime)].w, self.anime[self.frame % len(self.anime)].h, 0, 'h', self.x, self.y, 300, 110)
    skill.Draw = Draw_override.__get__(skill, Skill)

    def apply_effect_override(self, target, atk):
        for t in target:
            damage = atk*1.5 - t.status["def"]/2
            if damage <= 10:
                damage = 10
