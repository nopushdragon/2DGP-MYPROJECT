...
enya = Character([
    [load_image(f'source\\character\\enya\\enya04_0{i}.png') for i in range(1, 3)],
    [load_image(f'source\\character\\enya\\enya04_0{i}.png') for i in range(3, 5)],
    [load_image(f'source\\character\\enya\\enya04_0{i}.png') for i in range(5, 8)]
], 100, 400, [],name = "에냐", get = True,flip= True)

enya.illust = load_image('source\\character\\enya\\hero_illust_12_Enya.png')

enya.status = {"nowhp": 100, "maxhp":650, "atk": 50, "def":20, "speed": 110, "condition":[], "origin_atk":100, "origin_def" : 60, "origin_speed":140}  # nowhp, maxhp, attack, speed
...
enya.skill_2_sound = load_wav('source\\sound\\enya2.mp3')
enya.skill_2_sound.set_volume(32)
enya.skill_3_sound = load_wav('source\\sound\\balbar2.mp3')
enya.skill_3_sound.set_volume(32)
